...
const Packages = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let url = "/packages";
                if (state) {
                    // First try with state parameter
                    url += `?state=${state}`;
                    ...
                    if (
                        stateResponse.data.success &&
                        stateResponse.data.packages &&
                        stateResponse.data.packages.length > 0
                    ) {
                        setPackageData(stateResponse.data.packages);
                        setLoading(false);
                        return;
                    }

                    // If no packages found with state, try with search parameter
                    url = `/packages?search=${state}`;
                    const searchResponse = await api.get<{
                        success: boolean;
                        packages: PackageData[];
                    }>(url);

                    if (searchResponse.data.success && searchResponse.data.packages) {
                        setPackageData(searchResponse.data.packages);
                    } else {
                        setPackageData([]);
                    }
                } else {
                    // If no state parameter, fetch all packages
    ...
    const filteredPackages =
        selectedCategory === "all" ? packageData : packageData.filter((pkg) => pkg.features.includes(selectedCategory));
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white flex items-center justify-center">
                <div className="text-center bg-white p-8 rounded-2xl shadow-lg">
                    ...
                    <p className="text-gray-700 font-medium text-lg">Loading amazing destinations...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div className="flex items-center space-x-3">
                            ...
                            <span className="text-gray-800 font-semibold text-lg">Filter by category:</span>
                        </div>
